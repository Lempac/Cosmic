...
export type LexerToken = {
  type: keyof typeof tokenTypes | "EndOfFile";
  value: unknown;
  start: number;
  end: number;
};

export function parseToTokens(
  input: string,
  debugTokensToFile: boolean,
): Result<LexerToken[], CosmicError[]> {
  const start = performance.now();
  input += "\n";
  let position = 0;
  const tokens: LexerToken[] = [];
  let hasFailed = false;
  const issues: CosmicError[] = [];
  ...
  const filtered = tokens.filter((token) =>
    !tokensToExclude.includes(token.type)
  );
  ...
  if (debugTokensToFile) {
    ...
    Deno.writeTextFile("./debug/tokens.jsonc", res);
  }
