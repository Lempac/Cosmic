...
export class BooleanLiteralRule implements Rule {
  matches(
    ruleSet: RuleSet,
    tokens: LexerToken[],
  ): Result<BooleanNode, CosmicError> {
    if (tokens.length === 0) throw new Error("No tokens!");
    const token = tokens.shift()!;
...
export class IdentifierRule implements Rule {
  ...
  matches(
    ...
    if (token.type === "Identifier") {
      if (token.value === this.identifier) {
        return Ok({
          type: "Identifier",
          value: token.value,
          start: token.start,
      ...
      return Err({
        type: "UnexpectedSymbol",
        reason:
          `Expected symbol '${this.identifier}' instead got ${token.value}`,
        actualToken: token,
