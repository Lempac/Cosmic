...
export class Sequence implements Rule {
  ...
  matches(
    ruleSet: RuleSet,
    tokens: LexerToken[],
  ): Result<AstNode, CosmicError> {
    const result: { [key: string]: unknown } = {};
    let start: number | undefined = undefined;
    let end: number;

    if (this.rules.length === 0) {
      throw new Error("Unable to have sequence with no rules!");
    }
    let critical = false;
  ...
  toGrammar(ruleSet: RuleSet, depth: number): string {
    const ruleStrings = this.rules.map((rule) =>
      rule[1].toGrammar(ruleSet, depth + 1)
    );
...
export class AnyOf implements Rule {
  ...
  matches(
    ...
    return Err({
      ...
      reason: `Expected type of '${this.toGrammar(ruleSet, 0)}' instead got '${
        tokens[0].type
      }'`,
  ...
  toGrammar(ruleSet: RuleSet, depth: number): string {
    const ruleStrings = this.rules.map((rule) =>
      rule.toGrammar(ruleSet, depth + 1)
    );
...
export class BinaryOp implements Rule {
  ...
  matches(
    ...
    const result = match.unwrap() as unknown as {
      lhs: AstNode;
      rhs_matches: AstNode[];
    };
    ...
    for (let i = 0; i < result.rhs_matches.length; i++) {
      const match = result.rhs_matches[i] as unknown as {
        op: SymbolNode;
        rhs: AstNode;
      };
